...
public function duplicateForInstructor($courseId, $instructorId)
{
    DB::beginTransaction();

    try {
        $original = Course::with(['lessons.lectures'])->findOrFail($courseId);

        $newCourse = $original->replicate();
        $newCourse->user_id = $instructorId;
        $newCourse->organization_id = null;
        $newCourse->status = 0;
        $newCourse->save();
        ...
        foreach ($original->lessons as $lesson) {
            ...
            foreach ($lesson->lectures as $lecture) {
                ...
                $newLecture->lesson_id = $newLesson->id;
                $newLecture->course_id = $newCourse->id;
                $newLecture->save();
            }
        }

        // Duplicar quizzes del curso original
        $originalExams = Exam::with(['questions.options'])->where('course_id', $original->id)->get();

        foreach ($originalExams as $exam) {
        ...
        return redirect()->route('organization.course.index')->with('success', 'Curso duplicado para el instructor.');
    } catch (\Exception $e) {
        DB::rollBack();
        return redirect()->route('organization.course.index')->with('error', 'Error: ' . $e->getMessage());
    }
}
